...
def set_target_pose_to_object(
    ...
    asset_cfg: SceneEntityCfg = SceneEntityCfg("cube1"),
...
def random_set_target_pose(
    ...
    choice_ids = torch.randint(len(candidates), (len(env_ids),), device=env.device)
    for candidate_idx, (asset_name, height_offset) in enumerate(candidates):
        mask = choice_ids == candidate_idx
        if torch.any(mask):
            set_target_pose_to_object(
                env,
                env_ids[mask],
...
def keep_gripper_open(
    ...
    target = torch.full((len(env_ids), len(joint_ids)), open_pos, device=env.device, dtype=robot.data.joint_pos.dtype)
    joint_pos = robot.data.joint_pos[env_ids].clone()
    joint_vel = robot.data.joint_vel[env_ids].clone()
    joint_pos[:, joint_ids] = open_pos
